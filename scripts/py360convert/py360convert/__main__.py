...
def main():
    ...
    p2e_parser = subparsers.add_parser("p2e", help="Convert perspective to equirectangular.", add_help=False)
    p2e_parser.add_argument("--help", action="help", help="Show this help message and exit")
    p2e_parser.add_argument("input", type=Path, help="Path to input image.")
    p2e_parser.add_argument("output", type=Path, help="Path to output image.")
    p2e_parser.add_argument("--height", "-h", type=int, required=True, help="Output image height in pixels.")
    p2e_parser.add_argument("--width", "-w", type=int, required=True, help="Output image width in pixels.")
    p2e_parser.add_argument("--h-fov", type=float, default=60, help="Horizontal FoV in degrees.")
    p2e_parser.add_argument("--v-fov", type=float, default=60, help="Vertical FoV in degrees.")
    p2e_parser.add_argument(
        "--yaw",
        type=float,
        default=0,
        help="Yaw camera left/right degrees. Positive values rotate right; negative values rotate left.",
    ...
    if args.command is None:
        ...
        Image.fromarray(out).save(args.output)
    elif args.command == "p2e":
        _assert_height_width(args)
        img = np.array(Image.open(args.input))
        out = py360convert.p2e(
            img,
            args.height,
            args.width,
            fov_deg=(args.h_fov, args.v_fov),
